"""
xLSTM Anomaly Detection - Orijinal LSTM kodunuzla AYNI yapı
SADECE LSTMForecast class'ı xLSTMForecast olarak değiştirildi
"""
...
# ========= xLSTM Model (SADECE BU KISIM FARKLI) =========
...
def plot_learning_curve(train_losses, val_losses, best_epoch=None):
    ...
    ax1.set_title("xLSTM Eğitim Kaybı", fontsize=14, fontweight='bold')
    ...
    ax2.set_yscale('log')
    ax2.set_title("xLSTM Eğitim Kaybı (Log Scale)", fontsize=14, fontweight='bold')
    ...
    plt.tight_layout()
    plt.savefig("xlstm_learning_curve.png", dpi=150)
    print("✅ Grafik kaydedildi: xlstm_learning_curve.png")
    plt.close()
...
def plot_learning_rate(lr_history):
    ...
    plt.tight_layout()
    plt.savefig("xlstm_lr_schedule.png", dpi=150)
    print("✅ Learning rate grafiği kaydedildi: xlstm_lr_schedule.png")
    plt.close()
...
def main():
    ...
    # ✨ xLSTM model (SADECE BU SATIR FARKLI)
    model = xLSTMForecast(input_dim=train_scaled.shape[1],
                          hidden=HIDDEN_SIZE,
    ...
    print(f"\n🚀 xLSTM Eğitim Başlıyor (Max {EPOCHS} epochs)...")
    ...
    with open("xlstm_scaler.pkl", 'wb') as f:
        pickle.dump(scaler, f)

    with open("xlstm_config.pkl", 'wb') as f:
        pickle.dump(config, f)

    print("✅ Model kaydedildi: best_xlstm_model.pth")
    print("✅ Scaler kaydedildi: xlstm_scaler.pkl")
    print("✅ Config kaydedildi: xlstm_config.pkl")
    ...
    if np.sum(y_true_val) == 0:
        ...
        print("=" * 70)
        print("\n⚠️  TEST SETİ henüz kullanılmadı - test ile test edin!")
