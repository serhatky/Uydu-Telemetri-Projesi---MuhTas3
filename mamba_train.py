"""
MAMBA ANOMALY DETECTION TRAINING
State Space Model (S4) tabanlı modern architecture
"""
...
class MambaBlock(nn.Module):
    ...
    Based on State Space Models (S4)
    """
    ...
    def forward(self, x):
        ...
        x, z = xz.chunk(2, dim=-1)  # (batch, seq, d_inner) each
        ...
        # Simplified SSM (state space model)
        # This is a simplified version for practicality
        dt = F.softplus(self.dt_proj(x))  # (batch, seq, d_inner)

        # Discretize and apply SSM
        A_bar = torch.exp(self.A.unsqueeze(0).unsqueeze(0) * dt.unsqueeze(-1))  # Discretization
...
class MambaForecast(nn.Module):
    """
    Mamba-based forecasting model for anomaly detection
    """
...
def plot_learning_curve(train_losses, val_losses, best_epoch=None):
    ...
    ax1.set_title("Mamba Eğitim Kaybı", fontsize=14, fontweight='bold')
    ...
    ax2.set_yscale('log')
    ax2.set_title("Mamba Eğitim (Log Scale)", fontsize=14, fontweight='bold')
    ...
    plt.tight_layout()
    plt.savefig("mamba_learning_curve.png", dpi=150)
    print("✅ Grafik kaydedildi: mamba_learning_curve.png")
    plt.close()
...
def plot_learning_rate(lr_history):
    ...
    plt.tight_layout()
    plt.savefig("mamba_lr_schedule.png", dpi=150)
    print("✅ Learning rate grafiği kaydedildi: mamba_lr_schedule.png")
    plt.close()
...
def main():
    ...
    print("=" * 70)
    print("🐍 MAMBA ANOMALY DETECTION TRAINING")
    print("=" * 70)
    print("✨ Mamba Features:")
    print("   • State Space Model (S4)")
    ...
    print(f"\n   📊 Train  : {len(raw_train):,} satır")
    print(f"   📊 Val    : {len(raw_val):,} satır")
    print(f"   📊 Test   : {len(raw_test):,} satır")
    ...
    # Scaler ONLY on train
    print("\n📏 Scaling (FIT ONLY ON TRAIN)...")
    scaler = MinMaxScaler(feature_range=(0, 1))
    scaler.fit(clean_train)

    print(f"   ✅ Scaler fitted on {len(clean_train):,} training samples")
    ...
    # Mamba model
    model = MambaForecast(input_dim=train_scaled.shape[1],
                          hidden=HIDDEN_SIZE,
                          num_layers=NUM_LAYERS,
                          dropout=DROPOUT,
                          d_state=16).to(DEVICE)
    ...
    print(f"\n🚀 Mamba Eğitim Başlıyor ({EPOCHS} epochs)...")
    print("=" * 70)
    ...
    with open("mamba_scaler.pkl", 'wb') as f:
        pickle.dump(scaler, f)

    with open("mamba_config.pkl", 'wb') as f:
        pickle.dump(config, f)

    print("✅ Model kaydedildi: best_mamba_model.pth")
    print("✅ Scaler kaydedildi: mamba_scaler.pkl")
    print("✅ Config kaydedildi: mamba_config.pkl")
    ...
    if np.sum(y_true_val) == 0:
        print("⚠️  Validation'da anomali yok.")
    else:
        print(f"ℹ️  {np.sum(y_true_val):,} anomali penceresi var.")
        f1, prec, rec, _ = find_best_threshold_and_report(val_losses_smoothed, y_true_val)

        print("\n" + "=" * 70)
        print("📋 MAMBA EĞİTİM ÖZETİ")
        print("=" * 70)
